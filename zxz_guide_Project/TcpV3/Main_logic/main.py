...
from TcpV3.Gui.main_ui import TcpMyui
from Universal_Methods.Universal_Method import Common_Method
from TcpV3.communication.Server import Server
from TcpV3.communication.Client import Client
...
class Tcplogic(TcpMyui):
    def __init__(self):
        ...
        self.initialize_ui_data_update_to_data_transfer_connect_signal()
    ...
    def initialize_Singal_connect_to_groove_fun(self):
        ...
        self.signal_center._QMessageBox.connect(self.sin_get_QMessageBox)

    # UI控件自带信号，一旦修改数据，触发更新数据
    def initialize_ui_data_update_to_data_transfer_connect_signal(self):
    ...
    def setClient(self):
        ...
        if ip == "...": ip = "127.0.0.1"
        if host_name == "":
            QMessageBox.warning(self, "警告", "必须填写host_name!", QMessageBox.Ok)
            return
        elif host_name in self.server_host_names:
            self.Ledit_host_name.clear()
            QMessageBox.warning(self, "警告", "host_name已存在请从命名!", QMessageBox.Ok)
            return
    ...
    def sin_get_QMessageBox(self, hint):
        r = QMessageBox.warning(self, "警告", hint, QMessageBox.Ok)
        if r == QMessageBox.Ok:
            self.quit()
