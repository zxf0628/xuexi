        ...
        if bstart:
            ...
            if res:
                ...
                if planekey not in planes:
                    ...
                    join_oneplane_times = [(int(x[0] + old_time), float(x[1])) for x in oneplane]
                    planes[planekey]["data"].extend(join_oneplane_times)

                    join_oneplane = [(int(x[0]), float(x[1])) for x in oneplane]
                    planes[planekey]["flow_data"].extend(join_oneplane)

                    times = (join_oneplane_times[-1][0] - join_oneplane_times[0][0]) / 1000
                    planes[planekey]["times"] = planes[planekey]["times"] + times
                    print("重复刀位时间叠加：",planes[planekey]["times"])



                    # all_data = planes[planekey]["data"]
                    # cut_data = all_data[5:-5]
                    # planes[planekey]["cut_data"] = cut_data
                    # cut_time = (cut_data[-1][0] - cut_data[0][0]) / 1000
                    # planes[planekey]["cut_times"] = cut_time
...
def outputCSV_log_flow(allplanedata, title):
...
def outputCSV_cut_planedata(allplanedata, title):
    plankeys = list(allplanedata.keys())
    plankeys.sort()
    count = len(plankeys)
    ...
    with open(title + "剔除前后5点" + '.csv', 'w', newline='', encoding='utf-8-sig') as f:
        writer = csv.writer(f)
        writer.writerow(headers)
        for r in range(maxrow):
            row = []
...
def outputCSV_all_planedata(allplanedata, title):
    plankeys = list(allplanedata.keys())
    plankeys.sort()
    count = len(plankeys)
...
def drawFigure_all_planedata(allplanedata, title):
    plankeys = list(allplanedata.keys())
    plankeys.sort()
    count = len(plankeys)
...
def drawFigure_cut_planedata(allplanedata, title):
    ...
    new_titles = [i[0] + " 总用时:" + str(i[1]) + "秒" for i in
                  zip(titles, titles_times)]
    print("zxf drawFigure_cut_planedata  titles:", new_titles)
    ...
    # Add figure title
    fig.update_layout(
        title_text=title + "剔除前后5点", yaxis_range=[-1.5, 1.5],
        height=1200,
    )
...
def handleLog(logname):
    ...
    outputCSV_log_flow(allplanedata, logname)
    outputCSV_cut_planedata(allplanedata, logname)
    outputCSV_all_planedata(allplanedata, logname)

    drawFigure_cut_planedata(allplanedata, logname)
    drawFigure_all_planedata(allplanedata, logname)
