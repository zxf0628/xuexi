        ...
        if bstart:
            ...
            if res:
                ...
                if planekey not in planes:
                    ...
                    print("zxf旧列表拼接上时间后：",join_oneplane)

                    print("zxf刀位数据以存在：", planes[planekey]["data"])
                    print("zxf现在读取刀位的数据", oneplane)
                    planes[planekey]["data"].append(("重复截此刀位 ms", "重复截此刀位 mm"))
                    planes[planekey]["data"].extend(join_oneplane)

                    # print("zxf类型类型", type(join_oneplane[-1][0]),type(planes[planekey]["times"][-1][0]))
                    # print("zxf数值", join_oneplane[-1][0],planes[planekey]["times"][-1][0])
                    # times = (join_oneplane[-1][0] - type(planes[planekey]["times"][-1][0]))/1000
                    # planes[planekey]["times"] = times
