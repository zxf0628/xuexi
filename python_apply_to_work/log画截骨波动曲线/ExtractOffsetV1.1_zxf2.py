    ...
    for ln in loglns:
        ...
        if bstart:
            ...
            if res:
                ...
                mm_split_list = split_list(mm_list, 4)
                mm_avg_list = avg_list(mm_split_list)
                print("zxf:", mm_avg_list)

                planes[planekey] = {"name": planename, "data": oneplane, "avg": mm_avg_list}
                print(planekey, planename, len(oneplane))

    print("zxfffffff:",planes)
    return planes
...
def outputCSV(allplanedata, title):
    ...
    lengths = []
    for k in plankeys:
        # headers.append(cnPlaneNames[k] + '截骨平面')
